...
public class MDBVerticle {
  ...
  private static void insertBreakdown(RoutingContext routingContext, EventBus eventBus) {
    logger.info("Adding Breakdown");
    JsonObject json = routingContext.getBodyAsJson();
    eventBus.request("mongodb.postCollection", json, ar -> {
      if (ar.succeeded()) {
        logger.info("Received result for inserting breakdowns {}", ar.result().body().toString());
  ...
  private static void listBreakdowns(RoutingContext routingContext, EventBus eventBus) {
    ...
    eventBus.request("mongodb.getCollections", COLLECTION_NAME, ar -> {
      if (ar.succeeded()) {
